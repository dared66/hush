...
std::string CFStringToStdString(CFStringRef s) {
    ...
    if (!CFStringGetCString(s, buffer, length, kCFStringEncodingUTF8)) buffer[0] = 0;
...
OSStatus ProxyAudioDevice::Initialize(AudioServerPlugInDriverRef inDriver, AudioServerPlugInHostRef inHost) {
    ...
    audioOutputQueue = dispatch_queue_create("local.hush.AudioDevice.audioOutputQueue", priorityAttribute);
...
#pragma unused(inClientProcessID, inQualifierDataSize, inQualifierData)
    ...
    //    property in the GetDevicePropertyData() method.
    switch (inAddress->mSelector) {
        case kAudioObjectPropertyCustomPropertyInfoList: *outDataSize = 3 * sizeof(AudioServerPlugInCustomPropertyInfo); break;
        case 'huid':
        case 'hrdy':
        case 'hmet': *outDataSize = sizeof(CFStringRef); break;
    ...
    switch (inAddress->mSelector) {
        case kAudioObjectPropertyCustomPropertyInfoList: {
            AudioServerPlugInCustomPropertyInfo info[3] = {
                {'huid', kAudioServerPlugInCustomPropertyDataTypeCFString, kAudioServerPlugInCustomPropertyDataTypeNone},
                {'hrdy', kAudioServerPlugInCustomPropertyDataTypeCFString, kAudioServerPlugInCustomPropertyDataTypeNone},
                {'hmet', kAudioServerPlugInCustomPropertyDataTypeCFString, kAudioServerPlugInCustomPropertyDataTypeNone}
            };
            UInt32 size = std::min(inDataSize, (UInt32)sizeof(info));
            memcpy(outData, info, size); *outDataSize = size; break;
        }
        case 'huid': {
            ...
            *((CFStringRef *)outData) = CFStringCreateCopy(NULL,
                outputDeviceReady.load() && hushOutputUID ? hushOutputUID : CFSTR(""));
            *outDataSize = sizeof(CFStringRef); break;
        }
        case 'hrdy':
            if (inDataSize < sizeof(CFStringRef)) return kAudioHardwareBadPropertySizeError;
            *((CFStringRef *)outData) = CFStringCreateWithFormat(NULL, NULL, CFSTR("%u"), outputDeviceReady.load() ? hushOutputID.load() : 0);
            *outDataSize = sizeof(CFStringRef); break;
        case 'hmet':
            if (inDataSize < sizeof(CFStringRef)) return kAudioHardwareBadPropertySizeError;
...
void ProxyAudioDevice::setupTargetOutputDevice() {
    ...
    if (newOutputDevice.isValid()) {
        ...
        hushOutputID.store(newOutputDevice.id);
        if (hushOutputUID) { CFRelease(hushOutputUID); hushOutputUID = nullptr; }
        AudioObjectPropertyAddress uidAddress = {kAudioDevicePropertyDeviceUID, kAudioObjectPropertyScopeGlobal, 0};
        UInt32 uidSize = sizeof(hushOutputUID);
        AudioObjectGetPropertyData(newOutputDevice.id, &uidAddress, 0, nullptr, &uidSize, &hushOutputUID);
...
#pragma unused(inInputTime)
    ...
    for (UInt32 bufferIndex = 0; bufferIndex < outOutputData->mNumberBuffers; bufferIndex++) {
        ...
        for (UInt32 channelIndex = 0; channelIndex < numChannelsToProcess; channelIndex++) {
            ...
            for (UInt32 frame = 0; frame < outOutputData->mBuffers[bufferIndex].mDataByteSize; frame += framesize) {
                float sample = std::isfinite(*in) ? *in : 0;
                float adjusted = sample * ((channelIndex == 0) ? volumeFactorL : volumeFactorR);
                *out += adjusted;
                inputPeak = std::max(inputPeak, std::abs(sample));
                outputPeak = std::max(outputPeak, std::abs(adjusted));
    ...
    hushInputPeak.store(inputPeak);
    hushOutputPeak.store(outputPeak);
    hushCallbacks.fetch_add(1);
    return noErr;
}
...
CFStringRef ProxyAudioDevice::copyDeviceNameFromStorage()
{
    ...
    if (result == NULL) {
        ...
        result = CFBundleCopyLocalizedString(
            bundle, CFSTR("DeviceName"), CFSTR("Hush"), CFSTR("Localizable"));
    }

    if (result == NULL) {
        result = CFStringCreateCopy(NULL, CFSTR("Hush"));
    }
...
void ProxyAudioDevice::setOutputDevice(CFStringRef deviceUID) {
    if (!gPlugIn_Host || !deviceUID || CFEqual(deviceUID, CFSTR(kDevice_UID))) {
        return;
    }
